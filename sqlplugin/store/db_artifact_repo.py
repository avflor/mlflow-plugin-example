...
from sqlplugin.store.dbmodels.initial_artifact_store_models import Base as InitialBase
from sqlplugin.store.dbmodels.initial_artifact_store_models import SqlArtifact
...
class DBArtifactRepository(ArtifactRepository):
    ...
    def list_artifacts(self, path):
        ...
        """
        path = os.path.join(self.root, path) if path is not None else self.root
        with self.ManagedSessionMaker() as session:
